...
$(function () {
    var originalData = $('#inquiries-information').serialize();

    $('#inquiries-information').submit(function (e) {
        e.preventDefault();
                ...
                if (response.action === 'update') {
                    if (dataChanged) {
                        Swal.fire({
                            ...
                            text: "কর্তৃপক্ষের জিজ্ঞাসা তথ্য আপডেট করছেন!",
                            icon: 'warning',
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.action === 'update' || response.action === 'insert') {
                    // Error message
                    $('#inquiries-alert span')
                        .html(response.message);
                    $('#inquiries-alert').fadeIn();
                    setTimeout(function () {
                        $('#inquiries-alert').fadeOut();
                    }
                    , 5000);
                    goToStep(9);
