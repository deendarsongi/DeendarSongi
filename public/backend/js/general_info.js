$(function () {
    var originalData = $('#general-information').serialize();
                ...
                if (response.action === 'update') {
                    if (dataChanged) {
                        Swal.fire({
                            ...
                            text: "সাধারণ তথ্য আপডেট করতে যাচ্ছেন!",
                            icon: 'warning',
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.action === 'update' || response.action === 'insert') {
                    //biodata id
                    if (response.biodata_id) {
                        $('#biodata-id').text(response.biodata_id);
                    }

                    //error message
                    $('#general-alert span').html(response.message);
                    $('#general-alert').fadeIn();
                    setTimeout(function () {
                        $('#general-alert').fadeOut();
                    }, 5000);

                    let btn = $('#biodata-preview-btn');
                    btn.removeClass('disabled-link');
                    btn.attr('href', '/preview/biodata');
