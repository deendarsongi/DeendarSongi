...
$(function () {
    var originalData = $('#profession-information').serialize();

    $('#profession-information').submit(function (e) {
        e.preventDefault();
                ...
                if (response.action === 'update') {
                    if (dataChanged) {
                        Swal.fire({
                            ...
                            text: "আপনি পেশাগত তথ্য আপডেট করতে যাচ্ছেন!",
                            icon: 'warning',
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.action === 'update' || response.action === 'insert') {
                    // Error message
                    $('#profession-alert span')
                        .html(response.message);
                    $('#profession-alert').fadeIn();
                    setTimeout(function () {
                        $('#profession-alert').fadeOut();
                    }
                    , 5000);
                    goToStep(6);
